...
import (
	...
	ohlcHandler "solution_ch_part1/src/interface/rest/handlers/ohlc"

	"solution_ch_part1/src/interface/rest/response"
	"solution_ch_part1/src/interface/rest/route"
...
func makeRoute(
	...
	// hh := ohlcHandler.NewHealthHandler(respClient)
	// r.Mount("/", route.HealthRouter(hh))
	ah := ohlcHandler.NewOHLCHandler(respClient, useCases.OHLCUC)

	r.Route("/api", func(r chi.Router) {

		r.Mount("/ohlc", route.OHLCRouter(ah))
	})
